...
def build_country_year_metrics(data: pd.DataFrame) -> pd.DataFrame:
...
def build_representative_index_data(metrics: pd.DataFrame) -> pd.DataFrame:
    """Create country-specific ASK/RPK indices with each country's own base year."""
    rows: list[pd.DataFrame] = []
    for code in REPRESENTATIVE_COUNTRIES:
...
def build_country_dynamic_metrics(
    ...
    country = country.loc[country["n_valid"].ge(MIN_DYNAMIC_VALID_YEARS)].copy()
    if country.empty:
