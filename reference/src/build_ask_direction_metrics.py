...
from .config import ASK_DIRECTION_END, ASK_DIRECTION_START, TOP_DIRECTION_N


def build_direction_metrics(direction: pd.DataFrame) -> pd.DataFrame:
    """Use pairwise-valid years so outbound and inbound means share the same years."""
    sample = direction.loc[
        direction["year"].between(ASK_DIRECTION_START, ASK_DIRECTION_END)
...
def select_direction_extremes(direction_metrics: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    low = (
        direction_metrics.sort_values(["ln_R", "country_code"], ascending=[True, True])
        .head(TOP_DIRECTION_N)
        .copy()
    )
    high = (
